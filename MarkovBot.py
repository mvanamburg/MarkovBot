...
async def on_message(message):
	if message.content.startswith('!markov') and message.author != client.user:
		...
		updating_archive = open('updating_archive.log', 'w')
		...
		markov_target_parsed = open(markov_target+'.log', 'w')
		
		number_of_lines = 0
		updating_archive = open('updating_archive.log', 'r')
		...
		if number_of_lines == 0:
			...
			markov_output = open(markov_target+'.log', 'r').read()
